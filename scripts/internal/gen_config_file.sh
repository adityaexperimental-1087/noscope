...
IS_UNICA_CERT_AVAILABLE()
{
    ...
    if [[ "$(sha1sum "$SRC_DIR/security/monsterrom_platform.pk8" 2> /dev/null | cut -d " " -f 1)" == "$PLATFORM_KEY_SHA1" ]] && \
            [[ "$(sha1sum "$SRC_DIR/security/monsterrom_platform.x509.pem" 2> /dev/null | cut -d " " -f 1)" == "$OTA_KEY_SHA1" ]]; then
...
#
#   ROM_BUILD_TIMESTAMP
#     Integer containing the build timestamp in seconds, this is used by the MonsterROM Updates app.
